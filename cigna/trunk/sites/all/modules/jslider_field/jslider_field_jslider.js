(function($, Drupal) {
  ...
  var jslidersSlideChange = function(event, ui) {
    ...
    //var $r=$('.edit-si-range-value').attr('value');
    ...
    if (ui.value) {
     ...
     var region=$("input[name='selected_region']").attr('value');
     var ad_count=$("input[name='adult_count']").attr('value');
     var ch_count=$("input[name='child_count']").attr('value');
     var gender=$('input:radio[name=radio]:checked').val();
    
     $.ajax({
            url:"get_quote/ajax/slider/premium",
            type:"POST",
            data:{sum_insured_value1:setting.value,quote_region:region,adult_count:ad_count,child_count:ch_count},
